...
#define MEDIUM_EPC  0xCC

#define WET_EPC   0xEE

#define FERTIGATE_TAG   0xFF
...
#define PIN            6
#define RELAY_PIN      10
#define FERTIGATE_PIN  4
#define FERTIGATE_LED  11
...
//RSSI VALUE FOR THE RELAY TO TURN ON/OFF
#define RELAY_RSSI    -60
...
void setup()
{
  ...
  //Set up Relay pin to turn on
  pinMode(RELAY_PIN, OUTPUT);
  pinMode(FERTIGATE_PIN, INPUT_PULLUP);
  pinMode(FERTIGATE_LED, OUTPUT);
...
void loop()
{
   ...
   bool nfertigatePass = digitalRead(FERTIGATE_PIN);
   //digitalWrite(RELAY_PIN, false);
   
   if(nfertigatePass){
          
      digitalWrite(FERTIGATE_LED, false);
      }
   else{
          
      digitalWrite(FERTIGATE_LED, true);
      }
    ...
    else if (responseType == RESPONSE_IS_TAGFOUND)
    {
      ...
      if(freq < 1000000 && nano.getAntennaeIDNew() == 17){
         ...
         if(nfertigatePass){
          ...
          if(EPCHeader == DRY_EPC && EPCFertigate != FERTIGATE_TAG){

            if(moisture >= THRESHOLD_DRY && rssi >= RELAY_RSSI){
            ...
            else if(EPCHeader == MEDIUM_EPC && EPCFertigate != FERTIGATE_TAG){
              
              if(moisture >= THRESHOLD_MEDIUM && rssi >= RELAY_RSSI){
              ...
              else if(EPCHeader == WET_EPC && EPCFertigate != FERTIGATE_TAG){
      ...
      //Print EPC bytes, this is a subsection of bytes from the response/msg array
        ...
        Serial.print(EPCHeader, HEX);
        Serial.print(F(" "));

        Serial.print(F("]"));

        Serial.print(F(" epc_fertigate["));
        Serial.print(EPCFertigate, HEX);
        Serial.print(F(" "));
        Serial.print(F("]"));
